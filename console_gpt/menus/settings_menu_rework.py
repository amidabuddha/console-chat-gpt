...
from console_gpt.menus.skeleton_menus import base_settings_menu, base_multiselect_menu
from console_gpt.prompts.system_prompt import system_reply


def start_menu(allowed_settings: List[str] = ("features", "defaults", "models")):
    data = fetch_variable()
    allowed_settings = [x .capitalize()for x in allowed_settings]
    first_menu = base_multiselect_menu("Settings:", list(allowed_settings), "Settings:")
    print(base_multiselect_menu("Settings:", list(data[first_menu.lower()].keys()), "Settings:"))
