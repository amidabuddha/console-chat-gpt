...
def combined_menu() -> ChatObject | AssistantObject:
    ...
    assistant = assistant_menu(model) if model["model_title"].lower().startswith(("gpt","o1","o3")) else None
    if assistant:
        ...
        if not continue_chat:
            # Covering o1 models beta limitations. More info here: https://platform.openai.com/docs/guides/reasoning/beta-limitations
            if model["model_title"].lower() in ["o1-preview", "o1-mini"]:
                temperature = 1
                conversation = []
            else:
                _, role = role_menu()
