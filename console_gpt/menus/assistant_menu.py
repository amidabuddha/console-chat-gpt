...
def _save_assistant(model, role_title, assistant_id, thread_id=None):
        if not thread_id:
            thread_provided = (
                custom_input(
                message="Enter a preferred thread ID or press Enter to create a new one:",
            ))
            if thread_provided != '':
...
def _assistant_selection_menu(model):
    ...
    assistant_selection = base_multiselect_menu("Assistant menu", selection_menu, "Please select yor Assistant:", "Create New Assistant",preview_command=_assistant_preview)
